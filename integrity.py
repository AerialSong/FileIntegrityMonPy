...
def comp_baseline():
    ...
    if os.path.exists(jsfile): # Load file|hash from baseline.txt and store them in a dictionary
        ...
        # Next Step: Instead of repeatedly saying a file has been changed
        # Say it only once, add that filename and original hash to a log file
        # Change the baseline to include the hash of this new file
        while True:
            for subdir, dirs, files in os.walk(directory):
                for filename in files:
